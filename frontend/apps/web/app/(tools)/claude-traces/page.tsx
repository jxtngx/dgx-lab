...
export default function ClaudeTracesPage() {
  const { data, loading, refetch } = useFetch<ConversationsResponse>("/claude-agents/conversations")
  ...
  useEffect(() => {
    ...
    api<ConversationDetailResponse>(`/claude-agents/conversations/${selectedId}`)
      .then(setDetail)
      .catch(() => setDetail(null))
      .finally(() => setDetailLoading(false))

    api<SubagentsResponse>(`/claude-agents/conversations/${selectedId}/subagents`)
      .then(setSubagents)
  ...
  useEffect(() => {
    ...
    api<SubagentDetailResponse>(
      `/claude-agents/conversations/${selectedId}/subagents/${selectedSubagentId}`,
    )
  ...
  useEffect(() => {
    api<StatsResponse>("/claude-agents/stats")
      .then(setStats)
      .catch(() => {})
  }, [])
      ...
      {!hasConversations && !loading ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-3 text-text-tertiary">
          ...
          <p className="text-[13px] font-medium text-text-secondary">
            No Claude conversations yet
          </p>
          <p className="text-xs">
            Enable Claude Traces in the sidebar account menu
          </p>
        </div>
      ) : (
